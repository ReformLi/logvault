...
自托管 Vercel 访问日志管理平台，通过 Vercel Request Logs API 抓取日志，加密持久化存储，提供可视化查询与定时同步能力。
...
### 日志抓取
- **手动抓取**：页面按钮实时触发
- **定时抓取**：Vercel Cron Jobs 驱动，可配置间隔（30分钟/1小时/6小时/12小时/每日）
...
- 记录操作：登录（成功/失败）、手动抓取、删除、设置修改、定时抓取、数据清理
